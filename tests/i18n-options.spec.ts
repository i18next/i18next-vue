...
test.each([
    ...
    [false, 'Hello world'],
    [true, 'Hello world']
])('i18nOptions not supported with useTranslation', async (enableLegacy: boolean | undefined, expected: string) => {
    const Component = {
        template: '<p>{{ t(k) }}</p>',
        props: ['k'],
        setup() {
...
])('i18nOptions: namespaces %s (%s)', async (namespaces: string[] | string, enableLegacy: boolean | undefined, expected: string) => {
    const Component = {
        ...TranslateWithKey,
        i18nOptions: {
